...
const CourseStatsPanel: React.FC<CourseStatsPanelProps> = ({ termId, courseCode, hours = 72 }) => {
  ...
  if (
    !statsData ||
    (statsData.request_stats.total_requests === 0 && Object.keys(statsData.sections).length === 0)
  ) {
    return null;
  }
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
              <span className="text-xl font-bold leading-none">{request_stats.total_requests}</span>
              <span className="text-sm text-muted-foreground font-medium">Total Watches</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant="outline"
              className={cn(
                "text-xs py-1.5 px-3 font-bold gap-1.5 rounded-md border border-border/50 shadow-sm",
                demand.color
              )}
            >
              {demand.icon}
            ...
            {request_stats.requests_last_24h > 0 && (
              <Badge
                variant="secondary"
                className="text-[10px] font-bold bg-background text-muted-foreground border border-border/40 rounded-md"
              >

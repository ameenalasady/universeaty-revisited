...
export const WatchDashboard: React.FC = () => {
    ...
    const activeWatches = filteredWatches.filter(w => w.status !== 'cancelled' && !isOldTerm(w.term_id));
    const cancelledWatches = filteredWatches.filter(w => w.status === 'cancelled' && !isOldTerm(w.term_id));
                ...
                <div className="space-y-8">
                    ...
                    {oldTermWatches.length > 0 && (
                        <div>
                            ...
                            <div className="opacity-60">
                                {oldTermWatches.map(renderWatch)}
                            </div>
                        </div>
                    )}
                </div>

...
class RequestStorage:
    ...
    def _init_db(self):
        ...
        with self.db_lock:
            ...
            try:
                ...
                """)
                cursor.execute(
                    f"CREATE INDEX IF NOT EXISTS idx_status ON {self.WATCH_REQUESTS_TABLE}(status)"
                )
                cursor.execute(
                ...
                """)
                cursor.execute(
                    f"CREATE INDEX IF NOT EXISTS idx_snapshots_lookup ON {self.SEAT_SNAPSHOTS_TABLE}(term_id, course_code, section_key, recorded_at)"
                )
                cursor.execute(
                ...
                """)
                cursor.execute(
                    f"CREATE INDEX IF NOT EXISTS idx_auth_email ON {self.AUTH_TOKENS_TABLE}(email)"
                )
                ...
                if conn:
                    try:
                        ...
                        log.error(
                            f"Error closing DB connection during initialization: {close_err}"
                        )
    ...
    def check_connection(self) -> bool:
        ...
        with self.db_lock:  # Ensure thread safety
            ...
            try:
                ...
                log.debug(
                    f"RequestStorage connection check successful (took {duration:.3f}s)."
                )
                return True
            except sqlite3.Error as e:
                duration = time.time() - start_time
                log.error(
                    f"RequestStorage connection check failed (after {duration:.3f}s): {e}"
                )
                return False
            finally:
                if conn:
                    try:
                        ...
                        log.error(
                            f"Error closing DB connection during health check: {close_err}"
                        )

    def add_or_update_request(
        self,
        email: str,
        term_id: str,
        course_code: str,
        ...
        with self.db_lock:
            ...
            try:
                ...
                if existing_request:
                    ...
                    if existing_status == self.STATUS_PENDING:
                        ...
                        raise AlreadyPendingError(
                            course_code, section_display, existing_id, msg
                        )  # RAISE EXCEPTION

                    else:
                        # Found existing but non-pending (notified, error, etc.), reactivate it
                        log.info(
                            f"Storage: Found existing request (ID: {existing_id}, Status: {existing_status}). Reactivating to '{self.STATUS_PENDING}'."
                        )
                        cursor.execute(
                    ...
                    log.info(
                        f"Storage: No existing request found. Inserting new pending request for {email}, {term_id}, {section_key}."
                    )
                    cursor.execute(
                    ...
                    if (
                        request_id is None
                    ):  # Should not happen with autoincrement but check defensively
                ...
                raise DatabaseError(
                    message=msg, original_exception=e
                ) from e  # RAISE EXCEPTION
            finally:
                if conn:
                    try:
                        conn.close()
                        ...
                        log.error(
                            f"Storage: Error closing database connection after add/update: {close_err}"
                        )

    def add_or_update_batch_requests(
        self,
        email: str,
        term_id: str,
        course_code: str,
            ...
            try:
                ...
                for section in sections_data:
                    ...
                    if existing_request:
                        ...
                        if existing_request["status"] == self.STATUS_PENDING:
                            messages.append(
                                f"Request for {course_code} {section_display} is already pending (ID: {existing_id})."
                            )
                        else:
                            cursor.execute(
                            ...
                            messages.append(
                                f"Successfully reactivated your previous watch request (ID: {existing_id}) for {course_code} {section_display}."
                            )
                        ...
                        request_id = cursor.lastrowid
                        messages.append(
                            f"Successfully added new watch request (ID: {request_id}) for {course_code} {section_display}."
                        )
                ...
                if conn:
                    try:
                        ...
                        log.error(f"Storage: Error during rollback: {rb_err}")
                raise DatabaseError(message=msg, original_exception=e) from e
            finally:
                if conn:
                    try:
                        conn.close()
                        ...
                        log.error(
                            f"Storage: Error closing database connection after batch: {close_err}"
                        )
    ...
    def get_pending_requests(self) -> list[dict[str, Any]]:
        ...
        with self.db_lock:
            ...
            try:
                ...
                conn.close()
                log.debug(
                    f"Storage: Retrieved {len(pending_requests)} pending requests."
                )
            except sqlite3.Error as e:
                # Log error but return empty list to allow check loop to continue gracefully
                log.error(
                    f"Storage: Error fetching pending requests: {e}", exc_info=True
                )
                ...
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing database connection after fetching pending: {close_err}"
                        )
    ...
    def get_actively_tracked_courses(self, days: int = 14) -> list[dict[str, str]]:
        ...
        with self.db_lock:
            ...
            try:
                ...
                log.error(
                    f"Storage: Error fetching actively tracked courses: {e}",
                    exc_info=True,
                )
                tracked_courses = []
            finally:
    ...
    def update_request_statuses(
        self, notified_ids: list[int], error_ids: list[int], checked_ids: list[int]
    ):
        """
        ...
        log.debug(
            f"Storage: Preparing to update statuses. Notified: {len(notified_ids)}, Error: {len(error_ids)}, Checked: {len(checked_ids)}"
        )
        ...
        with self.db_lock:
            ...
            try:
                ...
                now_iso = datetime.now(
                    UTC
                ).isoformat()  # Consistent timestamp for the batch
                ...
                if notified_ids:
                    ...
                    log.info(
                        f"Storage: Updating status to '{self.STATUS_NOTIFIED}' for IDs: {unique_notified_ids}"
                    )
                ...
                if error_ids:
                    ...
                    log.info(
                        f"Storage: Updating status to '{self.STATUS_ERROR}' for IDs: {unique_error_ids}"
                    )
                ...
                remaining_checked_ids = [
                    id_ for id_ in valid_checked_ids if id_ not in processed_ids
                ]

                if remaining_checked_ids:
                    log.debug(
                        f"Storage: Updating last_checked_at for {len(remaining_checked_ids)} still-pending requests."
                    )
                ...
                log.info("Storage: Status updates committed.")

            except sqlite3.Error as e:
                log.error(
                    f"Storage: Database error updating watch request statuses: {e}",
                    exc_info=True,
                )
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error during rollback on update failure: {rb_err}"
                        )
        ...
        with self.db_lock:
            ...
            try:
                ...
                for snap in snapshots:
                    ...
                    if (
                        term_id is None
                        or course_code is None
                        or section_key is None
                        or open_seats is None
                        or total_seats is None
                    ):
                        continue
                    ...
                    if (
                        last_row
                        and last_row["open_seats"] == open_seats
                        and last_row["total_seats"] == total_seats
                    ):
                        # Values are identical. Check if it's been > 1 hour (heartbeat)
                        try:
                            # Parse 'YYYY-MM-DD HH:MM:SS' UTC format
                            last_recorded = datetime.strptime(
                                last_row["recorded_at"], "%Y-%m-%d %H:%M:%S"
                            )
                            last_recorded = last_recorded.replace(tzinfo=UTC)
                            if (
                                datetime.now(UTC) - last_recorded
                            ).total_seconds() < 3600:
                ...
                if inserted_count > 0:
                    log.info(
                        f"Storage: Recorded {inserted_count} new seat snapshots (out of {len(snapshots)} candidates)."
                    )
                else:
                    log.debug(
                        f"Storage: No new seat snapshots to record ({len(snapshots)} candidates checked, all unchanged)."
                    )
                return inserted_count

            except sqlite3.Error as e:
                log.error(
                    f"Storage: Error recording seat snapshots batch: {e}", exc_info=True
                )
                if conn:
                    try:
                        conn.rollback()
                        ...
                        log.error(
                            f"Storage: Error during rollback on snapshot batch: {rb_err}"
                        )
                return 0
            finally:
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing connection after snapshot batch: {close_err}"
                        )

    def get_section_history(
        self, term_id: str, course_code: str, section_key: str, hours: int = 72
    ) -> list[dict[str, Any]]:
        """
        ...
        with self.db_lock:
            ...
            try:
                ...
                log.debug(
                    f"Storage: Retrieved {len(results)} history points for {course_code}/{section_key} (last {hours}h)."
                )
            except sqlite3.Error as e:
                log.error(
                    f"Storage: Error fetching section history: {e}", exc_info=True
                )
            finally:
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing connection after history fetch: {close_err}"
                        )
        return results
            ...
            try:
                ...
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing connection after section stats: {close_err}"
                        )
        return stats

    def get_course_request_stats(
        self, term_id: str, course_code: str
    ) -> dict[str, Any]:
        """
        ...
        with self.db_lock:
            ...
            try:
                ...
                log.error(
                    f"Storage: Error computing course request stats: {e}", exc_info=True
                )
            finally:
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing connection after course request stats: {close_err}"
                        )
        return stats

    def get_course_sections_with_history(
        self, term_id: str, course_code: str, hours: int = 72
        ...
        with self.db_lock:
            ...
            try:
                ...
                log.error(
                    f"Storage: Error fetching section keys for course history: {e}",
                    exc_info=True,
                )
                return result
            finally:
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing connection after section keys fetch: {close_err}"
                        )

        # Fetch history and stats per section (releases lock between calls)
        for section_key in section_keys:
            result[section_key] = {
                "history": self.get_section_history(
                    term_id, course_code, section_key, hours
                ),
    ...
    def cleanup_old_snapshots(self, days: int = 30) -> int:
        ...
        with self.db_lock:
            ...
            try:
                ...
                if deleted_count > 0:
                    log.info(
                        f"Storage: Cleaned up {deleted_count} seat snapshots older than {days} days."
                    )
                else:
                    log.debug(
                        f"Storage: No seat snapshots to clean up (threshold: {days} days)."
                    )
            except sqlite3.Error as e:
                log.error(
                    f"Storage: Error cleaning up old snapshots: {e}", exc_info=True
                )
                if conn:
                    try:
                        conn.rollback()
                        ...
                        log.error(
                            f"Storage: Error during rollback on snapshot cleanup: {rb_err}"
                        )
            finally:
                if conn:
                    try:
                        ...
                        log.error(
                            f"Storage: Error closing connection after snapshot cleanup: {close_err}"
                        )
    ...
    def create_auth_token(
        self, email: str, raw_token: str, expires_in_minutes: int
    ) -> bool:

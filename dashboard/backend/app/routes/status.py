    ...
    return jsonify(
        {
            ...
            "ram": get_ram_usage(),
            "disk": get_disk_usage(),
            "service": get_service_status("universeaty.service"),
            "dashboard_service": get_service_status("universeaty-dashboard.service"),
            "db_size_mb": db_size,
...
def service_restart():
    ...
    if status_info == "active":
        return jsonify(
            {"status": "success", "message": "Service successfully restarted!"}
        )
    return jsonify(
        {
            "status": "error",
            "message": f"Service restart initiated, but status is: {status_info}. Error: {result}",
        }
    ), 500
